...
# - Susceptibility weighted imaging at 0.5, 1 and 2mm subcortically
# - grey-white matter intensity contrast
# 3. Calculate curvature for use in script 2
...
do
  ...
  do
    ...
    mris_convert -c "$s"/surf/"$h".pial.K.crv "$s"/surf/"$h".white "$s"/surf/"$h".pial.K.mgh
  done
done
